...
// KeyFieldParam Specifies which field is used to map the extract fields. If your document contains fields with the same name, then we suggest using the id field. Defaults to name.
type KeyFieldParam string

// List of keyFieldParam
const (
	KEYFIELDPARAM_NAME KeyFieldParam = "name"
	KEYFIELDPARAM_ID KeyFieldParam = "id"
)

// All allowed values of KeyFieldParam enum
var AllowedKeyFieldParamEnumValues = []KeyFieldParam{
	"name",
	"id",
}

func (v *KeyFieldParam) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := KeyFieldParam(value)
	for _, existing := range AllowedKeyFieldParamEnumValues {
	...
	return fmt.Errorf("%+v is not a valid KeyFieldParam", value)
}

// NewKeyFieldParamFromValue returns a pointer to a valid KeyFieldParam
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewKeyFieldParamFromValue(v string) (*KeyFieldParam, error) {
	ev := KeyFieldParam(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for KeyFieldParam: valid values are %v", v, AllowedKeyFieldParamEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v KeyFieldParam) IsValid() bool {
	for _, existing := range AllowedKeyFieldParamEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to keyFieldParam value
func (v KeyFieldParam) Ptr() *KeyFieldParam {
	return &v
}

type NullableKeyFieldParam struct {
	value *KeyFieldParam
	isSet bool
}

func (v NullableKeyFieldParam) Get() *KeyFieldParam {
	return v.value
}

func (v *NullableKeyFieldParam) Set(val *KeyFieldParam) {
	v.value = val
	v.isSet = true
}

func (v NullableKeyFieldParam) IsSet() bool {
	return v.isSet
}

func (v *NullableKeyFieldParam) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableKeyFieldParam(val *KeyFieldParam) *NullableKeyFieldParam {
	return &NullableKeyFieldParam{value: val, isSet: true}
}

func (v NullableKeyFieldParam) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableKeyFieldParam) UnmarshalJSON(src []byte) error {
	v.isSet = true
