...
// OutputParam Response format. When the \"file\" option is used the API returns the file inline. With the \"url\" and \"viewer\" option, the document is stored for 30 days and automatically deleted.
type OutputParam string

// List of outputParam
const (
	OUTPUTPARAM_BASE64 OutputParam = "base64"
	OUTPUTPARAM_URL OutputParam = "url"
	OUTPUTPARAM_FILE OutputParam = "file"
	OUTPUTPARAM_VIEWER OutputParam = "viewer"
)

// All allowed values of OutputParam enum
var AllowedOutputParamEnumValues = []OutputParam{
	"base64",
	"url",
	"file",
	"viewer",
...
func (v *OutputParam) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := OutputParam(value)
	for _, existing := range AllowedOutputParamEnumValues {
	...
	return fmt.Errorf("%+v is not a valid OutputParam", value)
}

// NewOutputParamFromValue returns a pointer to a valid OutputParam
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewOutputParamFromValue(v string) (*OutputParam, error) {
	ev := OutputParam(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for OutputParam: valid values are %v", v, AllowedOutputParamEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v OutputParam) IsValid() bool {
	for _, existing := range AllowedOutputParamEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to outputParam value
func (v OutputParam) Ptr() *OutputParam {
	return &v
}

type NullableOutputParam struct {
	value *OutputParam
	isSet bool
}

func (v NullableOutputParam) Get() *OutputParam {
	return v.value
}

func (v *NullableOutputParam) Set(val *OutputParam) {
	v.value = val
	v.isSet = true
}

func (v NullableOutputParam) IsSet() bool {
	return v.isSet
}

func (v *NullableOutputParam) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableOutputParam(val *OutputParam) *NullableOutputParam {
	return &NullableOutputParam{value: val, isSet: true}
}

func (v NullableOutputParam) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableOutputParam) UnmarshalJSON(src []byte) error {
	v.isSet = true
